...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for i in range(len(self.params)):
            param = self.params[i]
            m_prev = self.state[i]['m_t']
            v_prev = self.state[i]['v_t']
            m_t = b1 * m_prev + (1 - b1) * param.grad.data
            v_t = b2 * v_prev + (1 - b2) * np.square(param.grad.data)
            self.state[i]['m_t'] = m_t
            self.state[i]['v_t'] = v_t
            D = np.sqrt(v_t / B2) + self.eps
            param.data -= m_t * s_t / D
